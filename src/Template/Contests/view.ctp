...
$this->Breadcrumbs->add(__('Elections'), $contest->election->getBrowseRoute());
$this->Breadcrumbs->add($contest->election->name, $contest->election->getViewRoute());
$this->Breadcrumbs->add($contest->name);
?>
                    ...
                    <div class="col-md-12">
                        <dl>
                            ...
                            <?php if ($contest->electoral_district): ?>
                                <dt>Electoral district
                                    <small>the geographical scope of the contest</small>
                                </dt>
                            ...
                            <?php if ($contest->electorate_specification): ?>
                                <dt>Specifies any changes to the eligible electorate for this contest past the
                                    usual, “all registered voters” electorate. This subtag will most often be used
                                    for primaries and local elections. In primaries, voters may have to be
                                    registered as a specific party to vote, or there may be special rules for which
                                    ballot a voter can pull. In some local elections, non-citizens can vote.
                                </dt>
